...
.thm[data-m="light"] .i-light,.thm[data-m="dark"] .i-dark{display:block}
.thm{color:var(--soft)}
...
(function(){
  ...
  /* One click always flips what you can see. Cycling through "auto" meant a
     reader on a dark phone had to press twice to reach light, because auto
     and dark looked identical. Auto is still the starting state; the icon
     shows what the next click will do. */
  var tb=document.getElementById('thmBtn');
  function isDark(){
  ...
  function paintIcon(){
    ...
    tb.setAttribute('data-m',target);
    var lbl=target==='light'?'उजळ करा':'गडद करा';
    tb.setAttribute('aria-label',lbl); tb.setAttribute('title',lbl);
  }
  if(tb){
    tb.addEventListener('click',function(){
      var target=isDark()?'light':'dark';
      root.setAttribute('data-theme',target);
      try{localStorage.setItem('pn-theme',target);}catch(e){}
      paintIcon();
    });
    paintIcon();
